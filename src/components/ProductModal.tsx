...
const ProductModal: React.FC<ProductModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('itemMasterId', formData.itemMasterId);
      formDataToSend.append('inventoryType', productType);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex justify-end space-x-4 pt-4 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50 font-semibold flex items-center space-x-2 shadow-lg"
            >
              {loading ? (
                <>
